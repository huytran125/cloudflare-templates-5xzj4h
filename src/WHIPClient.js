...
export default class WHIPClient {
  ...
  async accessLocalMediaSources() {
    return navigator.mediaDevices
      ...
      .then(async (stream) => {
        // function to get camera height, width and calculate our stream width, height
        let constraints = {
          audio: true,
          video: {
            width: { ideal: 1920 },
        ...
        let stream_settings = newStream.getVideoTracks()[0].getSettings();
        ...
        stream.getTracks().forEach(async (track) => {
          ...
          if (track.kind == 'video' && transceiver.sender.track) {
            // 19.5:9 is current iphone ratio
            transceiver.sender.track.applyConstraints({
              width: (9 * stream_height) / 19.5,
